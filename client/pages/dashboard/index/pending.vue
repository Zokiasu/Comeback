<template>
	<div class="p-5 px-10 space-y-5">
		<section
			id="pending-page"
			class="pb-5 grid grid-cols-1 lg:grid-cols-2 gap-3"
		>
			<div
				v-for="(pending, index) in this.pendings.slice(0, maxObjectDisplay)"
				:key="index"
				style="background-color: #6b728033"
				class="flex flex-col text-white rounded-sm relative p-3 overflow-hidden space-y-2"
			>
				...
				<PendingArtist
					:pending="pending"
					v-if="
						pending.body.type === 'SOLO' ||
						pending.body.type === 'GROUP' ||
						pending.currentData.type === 'SOLO' ||
						pending.currentData.type === 'GROUP'
					"
				/>
				<PendingRelease
					:pending="pending"
					v-else-if="
						pending.body.type === 'SINGLE' ||
						pending.body.type === 'ALBUM' ||
						pending.body.type === 'EP' ||
						pending.currentData.type === 'SINGLE' ||
						pending.currentData.type === 'ALBUM' ||
						pending.currentData.type === 'EP'
					"
				/>
				<PendingMusic
					:pending="pending"
					v-else-if="
						!pending.body.type &&
						!pending.currentData.type &&
						(pending.body.clip || pending.currentData.clip)
					"
				/>
				<section v-else>
				...
				<section id="pending-button" class="flex space-x-3 justify-end">
					<button
						@click="editOpen(index)"
						class="bg-blue-500 px-2 py-1 focus:outline-none hover:bg-blue-700"
					>
						Edit
					</button>
					<button
						@click="accepted(pending, index)"
						class="bg-green-500 px-2 py-1 focus:outline-none hover:bg-green-700"
					>
						Accepted
					</button>
					<button
						@click="refused(pending, index)"
						class="bg-red-500 px-2 py-1 focus:outline-none hover:bg-red-700"
					>
						Refused
					</button>
				</section>
			...
			<div
				v-if="this.pendings.length < 1"
				style="background-color: #6b728033"
				class="w-full text-white lg:col-span-2 py-2 rounded-sm flex justify-center"
			>
...
export default {
	...
	async asyncData({ $axios }) {
		const artistList = await $axios.$get(
			"https://comeback-api.herokuapp.com/artists"
		);
		const styleList = await $axios.$get(
			"https://comeback-api.herokuapp.com/styles"
		);
	...
	methods: {
		...
		infiniteScroll($state) {
			...
			setTimeout(() => {
				...
				this.$axios
					.get(
						`https://comeback-api.herokuapp.com/requests?state=PENDING&limit=20&offset=${this.maxObjectDisplay}`
					)
					.then((response) => {
		...
		async accepted(object, index) {
			if (object.method == "PUT") {
				await this.$axios
					.put(
						`https://comeback-api.herokuapp.com${object.endpoint}`,
						object.body
					)
					.then((response) => {
						if (response)
		...
		async updateRequest(object, index, state, checked_by) {
			...
			await this.$axios
				.put(`https://comeback-api.herokuapp.com/requests/${object.id}`, object)
				.then((response) => {
					this.pendings.splice(index, 1);
					if (state == "ACCEPTED")
						this.$toast.success("The pending has been accepted", {
							duration: 3000,
							position: "top-right",
						});
					else
						this.$toast.error("The pending has been refused", {
							duration: 3000,
							position: "top-right",
						});
				})
