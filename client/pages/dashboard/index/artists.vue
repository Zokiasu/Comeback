<template>
	<div class="p-5 px-10 space-y-5">
		...
		>
			<div
				id="search-icon"
				class="bg-opacity-20 bg-gray-500 pr-1 pl-2 rounded-none rounded-l py-1.5"
			>
				...
				<ul class="flex flex-wrap gap-5">
					<li>
						<button
							...
							@click="onlySolo = false; onlyGroup = false;"
						>
							ALL
						</button>
					</li>
					<li>
						<button
							class="px-3 py-2 border rounded"
							:class="{'border-red-500 text-red-500 font-semibold border-2': onlyGroup}"
							@click="onlyGroup =! onlyGroup; onlySolo = false;"
						>
							GROUP
						</button>
					</li>
					<li>
						<button 
							class="px-3 py-2 border rounded" 
							:class="{'border-red-500 text-red-500 font-semibold border-2': onlySolo}"
							@click="onlySolo =! onlySolo; onlyGroup = false;"
						>
							SOLIST
						</button>
					</li>
		...
		>
			<div
				...
				class="flex flex-col text-white rounded-sm relative p-3 overflow-hidden"
			>
				<span class="absolute text-white bottom-0 right-0 bg-gray-900 px-2">{{
					index
				}}</span>
				...
				<div>
					<span
						...
						class="truncate hover:overflow-ellipsis break-words overflow-wrap block"
					>
						{{ artist.description }}
					</span>
		...
		<div v-if="artists.length < 1" class="px-5">
			<p
				style="background-color: #6b728033"
				class="text-white w-full flex justify-center rounded p-2"
				>No Artist found.</p>
		</div>
	</div>
</template>
	...
	computed: {
		filteredList() {
			return this.artists.filter(element => {
				if(this.onlyGroup && element.type.toLowerCase() != 'group') return false
