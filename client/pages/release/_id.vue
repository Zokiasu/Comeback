<template>
	<div class="text-white">
		<div
			class="background-top relative"
			:style="{ 'background-image': 'url(' + release.image + ')' }"
		>
			<div class="h-full w-full bg-background bg-opacity-30 flex">
				<div
					class="flex lg:space-x-10 mt-auto lg:my-auto w-full px-5 md:px-10 lg:px-20 z-50"
				>
					<div class="relative releaseJacket min-w-max">
						<img
							class="h-80 w-80 shadowRelease object-cover object-center"
							:src="release.image"
							:alt="release.name"
						/>
						<div
						...
						>
							<button
								@click="liked ? unfollowRelease() : followRelease()"
								:class="liked ? '' : ''"
					...
					<div class="mt-auto">
						<NuxtLink
							v-if="userInfo != null"
							:to="`/edit/release/${$route.params.id}`"
							class="mt-auto"
							>Edit</NuxtLink
						>
						<h2
							class="cursor-default font-semibold filter tShadowRelease text-xl md:text-2xl"
						>
							{{ release.type }} -
						...
						<h1
							class="cursor-default font-semibold filter tShadowRelease text-2xl md:text-4xl xl:text-5xl 2xl:text-6xl"
						>
							{{ release.name }}
						</h1>
						<div id="link-social" class="flex flex-row lg:space-x-10 pt-3">
							<LinkImg
								class="tShadowRelease"
		...
		<div class="py-5 lg:p-10">
			<div class="flex justify-between border-b border-white px-5 pr-8 pb-1">
				...
				>
					<path
						d="M9.5 15.5842V8.41583C9.5 8.02025 9.93762 7.78133 10.2704 7.99524L15.8458 11.5794C16.1519 11.7762 16.1519 12.2238 15.8458 12.4206L10.2704 16.0048C9.93762 16.2187 9.5 15.9798 9.5 15.5842Z"
						fill="white"
					/>
					<path
						fill-rule="evenodd"
	...
	methods: {
		...
		async followRelease() {
			...
			await this.$axios
				...
				.then((response) => {
					this.$toast.info("You are now following " + this.release.name, {
						duration: 2000,
		...
		async unfollowRelease() {
			...
			await this.$axios
				.delete(
					`https://comeback-api.herokuapp.com/users/${this.userInfo.id}/releases/${this.release.id}`,
					this.release
				)
				.then((response) => {
					this.$toast.info("You are now unfollowing " + this.release.name, {
						duration: 2000,
						position: "top-center",
					});
				})
