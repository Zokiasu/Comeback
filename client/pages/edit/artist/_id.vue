<template>
	<div class="p-10 text-white">
		<div class="border-b border-white flex justify-between">
			...
			<button
				@click="editArtist()"
				class="Card px-5 py-1 hover:bg-red-700 text-white rounded"
			>
				Confirm
			</button>
		</div>
		<section>
			...
			>
				...
				<div class="grid grid-cols-1 gap-6 w-full">
					...
					>
						...
						<t-input
							@change="newObjectToApi('name', artist.name)"
							autocomplete="false"
							type="text"
							v-model="artist.name"
							:value="artist.name"
					...
					>
						...
						<t-select
							@change="newObjectToApi('type', artist.type)"
							v-model="artist.type"
			...
			<div class="flex flex-col space-y-10">
				<div class="flex flex-col space-y-1">
					...
					<t-input
						@change="newObjectToApi('idyoutubemusic', artist.idyoutubemusic)"
						autocomplete="false"
						type="text"
				...
				<div class="flex flex-col space-y-1">
					...
					<t-textarea
						@change="newObjectToApi('description', artist.description)"
						id="desc"
				...
				<div class="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:gap-5">
					<div id="social-media" class="w-full space-y-1">
						...
						<button
							@click="addSocials()"
							class="w-full hover:bg-opacity-50 text-left focus:outline-none flex space-x-2 bg-white bg-opacity-30 p-2 justify-center rounded"
						>
	...
	methods: {
		...
		uploadImageFile(files) {
			...
			let imageRef = this.$fire.storage.ref(
				`images/artist/${this.artist.id}-${this.artist.name}`
			);
